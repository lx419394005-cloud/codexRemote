...
export function getBridgeBaseUrl() {
  if (typeof window !== 'undefined') {
    const runtimeDefault = `${window.location.protocol}//${window.location.hostname}:${bridgeDefaults.defaultBridgePort}`;
    return window.__CODEX_BRIDGE_BASE_URL__ || process.env.NEXT_PUBLIC_BRIDGE_URL || runtimeDefault;
  }
